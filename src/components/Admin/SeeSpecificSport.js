...
const SeeSportsData = () => {
  ...
  const fetchSportsData = async () => {
    setLoading(true);
    try {
      const response = await axios.post('http://localhost:3001/getSpecificSports/', { key });
  ...
  const checkExistingContests = async () => {
    try {
      const response = await axios.post('http://localhost:3001/alreadyExist');
  ...
  const submitContest = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/createContest', {
        id,
        title,
        sports_key: key,
        sport_title,
